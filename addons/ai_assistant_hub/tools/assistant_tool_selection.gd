...
func get_selection() -> String:
	...
	if _selected_code.strip_edges(true, true).length() == 0:
		var curr_line = clampi(_code_editor.get_caret_line(), 0, _code_editor.get_line_count() - 1)
...
func forget_selection() -> void:
	...
	_selected_code = ""
	_selected_code_first_line = ""
	_selected_code_last_line = ""
	_selected_code_line_start = 0
	_selected_code_line_start_column = 0
	_selected_code_line_end = 0
	_selected_code_line_end_column = 0
...
func back_to_selection() -> bool:
	...
	var pre_script_editor := editor_interface.get_script_editor()
	if pre_script_editor == null:
		return false
	var curr_script:Script = pre_script_editor.get_current_script()
