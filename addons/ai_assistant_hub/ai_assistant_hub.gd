...
func _initialize_llm_provider_options() -> void:
	...
	for provider_file in files:
		...
		if provider is LLMProviderResource:
			...
			i += 1
	if llm_provider_option.item_count > 0 and llm_provider_option.get_selected_id() == -1:
		llm_provider_option.select(0)
		_on_llm_provider_option_item_selected(0)
...
func _on_refresh_models_btn_pressed() -> void:
	if not url_txt.text.is_empty():
		if _models_llm == null:
			models_list_error.text = "LLM provider not loaded. Select a provider first."
...
func _on_llm_provider_option_item_selected(index: int) -> void:
	...
	if llm_provider == null:
		push_error("Invalid LLM provider selection.")
		return
	...
	if new_llm == null:
		...
		_models_llm = null
		models_list_error.text = "Selected LLM provider failed to initialize."
		models_list_error.visible = true
		models_list.visible = false
		return
	else:
