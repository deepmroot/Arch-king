...
func _ready() -> void:
	...
	wall_y = 536.0 # Match the top of the CastleWallBand

	player.wall_y = wall_y
	player.left_bound = 150.0 # Don't walk into the left tower
	player.right_bound = viewport_size.x - 150.0 # Don't walk into the right tower
	player.shoot_requested.connect(_on_player_shoot_requested)

	shop_zone.global_position = Vector2(viewport_size.x - 200.0, wall_y + 40.0)
	var wall_top := wall_y
	var wall_bottom := wall_y + 56.0
...
func _spawn_enemy() -> void:
	...
	enemy.global_position = Vector2(randf_range(24.0, screen_size.x - 24.0), -24.0)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if InputMap.has_action("interact") and event.is_action_pressed("interact") and player_in_shop_zone and not shop_panel.visible:
		_open_shop()
		get_viewport().set_input_as_handled()
